...
class NonlinearInterfaceSolver(InterfaceSolver):
    ...
    def F(self, snes, x, F):
        ...
        x.assemble()
        F.zeroEntries()
        if self.a_interface is not None:
        ...
        F_.apply('add')
        F1.update_ghost_values()
        F0.update_ghost_values()
        F_.axpy(1, F1)
        F_.axpy(1, F0)
    ...
    def J(self, snes, x, J, P):
        ...
        A0 = PETScMatrix()
        A1 = PETScMatrix()
        assemble(self.da0, tensor=A0, keep_diagonal=True, form_compiler_parameters=self.form_compiler_parameters)
        assemble(self.da1, tensor=A1, keep_diagonal=True, form_compiler_parameters=self.form_compiler_parameters)
        for bc in self.bcs_zero0:
            bc.zero(A0)
    ...
    def setup(self, a0, a1, a_interface,
        ...
        self.assembler.init_global_tensor(
            self.A_dolfin, Form(self.da0 + self.da1)
        )

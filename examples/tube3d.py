from dolfin import (
    ...
    inner, grad, dS, XDMFFile, FiniteElement, MixedElement, VectorElement, split,
    div, sym, Identity
)
...
inflow_inner =  CompiledSubDomain(
    "near(x[2], 0.0) && pow(x[0], 2) + pow(x[1], 2) <= pow(r, 2) + eps && on_boundary",
...
outflow_inner =  CompiledSubDomain(
    "near(x[2], 2.0) && pow(x[0], 2) + pow(x[1], 2) <= pow(r, 2) + eps && on_boundary",
    r=radius, eps=0.00001
)
outflow_outer =  CompiledSubDomain(
    "near(x[2], 2.0) && pow(x[0], 2) + pow(x[1], 2) > pow(r, 2) && on_boundary",
    r = radius
)
...
n = Expression(
    ...
    'x[1]/pow(pow(x[0], 2) + pow(x[1], 2), 0.5)', '0.0'), degree=4
)
mu1 = 1.0
mu0 = 2.0
...
solver.solve(a0, a1, a_interface,
             bcs0=[bc_in0], bcs1=[bc_in1, bc_cylinder], bcs_zero0=[], bcs_zero1=[bcm])
